...
import {OutlineButton, PostSearchSongsList} from '../..';
...
type SearchResultForPostSongProps = {
  inputText: string;
...
const SearchResultForPostSong = ({
  inputText,
  searchData,
  navigation,
}: SearchResultForPostSongProps) => {
  ...
  const renderItem = ({item}: {item: [string, Song[]]}) => {
    ...
    return (
      <View key={key} style={tw`my-4`}>
        ...
        <PostSearchSongsList songData={songs} />
      </View>
    );
  };
...
export {SearchResultForPostSong};
